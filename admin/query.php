<?php
    ...
    $sql = "SELECT * FROM reserve WHERE day_reserve = '$date'";
    ...
    //ดึงข้อมูล ห้อง(room) และให้ group by colum status
    $room_group_status = mysqli_query($con, "SELECT * FROM room GROUP BY status");

    //ดึงข้อมูล ห้อง(room) และให้ group by colum status
    $room_j_g_details_1 = mysqli_query($con, "SELECT * FROM `room`,`room_details` WHERE room.type = room_details.id and room_details.reservation = 1 GROUP BY status ");

    //ดึงข้อมูล ห้อง(room) และให้ group by colum status
    $room_j_g_details_2 = mysqli_query($con, "SELECT * FROM `room`,`room_details` WHERE room.type = room_details.id and room_details.reservation = 2 GROUP BY status ");
    ...
    //ดึงข้อมูล ธนาคาร
    $abank = mysqli_query($con,"SELECT bank.id as id , abank.img as img, abank.uname as uname,abank.bnumber as bnumber,bank.bname as bname FROM  abank INNER JOIN bank ON(bank.id = abank.b_id)");

    $bank = mysqli_query($con,"SELECT * FROM bank");
?>