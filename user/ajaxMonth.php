...
$urd1 = mysqli_query($con, "SELECT room_details.reservation, user.room, user.rental_start_date 
FROM user INNER JOIN room ON(user.room = room.room) INNER JOIN room_details ON(room.type = room_details.id)
...
INNER JOIN receipt ON(unit.room = receipt.room AND unit.date = receipt.date) 
INNER JOIN room_details ON(room.type = room_details.id) INNER JOIN user 
ON(user.room = receipt.room)  WHERE room.room = '$room' AND 
...
if (isset($_GET['reid'])) {
    ...
    $s1 = $_GET['reid']; 
    $findit = "SELECT * FROM room INNER JOIN unit ON(room.room = unit.room) 
    INNER JOIN receipt ON(unit.room = receipt.room AND unit.date = receipt.date) 
    INNER JOIN room_details ON(room.type = room_details.id)
    INNER JOIN user ON(user.room = receipt.room) WHERE room.room = '$room' 
    AND receipt.date LIKE '%$s1%' GROUP BY receipt.pay_id ";
...
while ($findtoo = $find->fetch_assoc()) :


$his1 = mysqli_query($con, " SELECT * FROM room INNER JOIN unit ON(room.room = unit.room)
 INNER JOIN receipt ON(unit.room = receipt.room AND unit.date = receipt.date) 
 INNER JOIN room_details ON(room.type = room_details.id) INNER JOIN user 
 ON(user.room = receipt.room)  WHERE room.room = '$room' AND  receipt.pay_id < 
('$findtoo[pay_id]') GROUP BY receipt.date ORDER BY receipt.date DESC LIMIT 1 ");
$history = mysqli_fetch_array($his1);

$ot = mysqli_query($con, " SELECT  receipt.pay_id,unit.other FROM room INNER JOIN unit 
ON(room.room = unit.room) INNER JOIN receipt ON(unit.room = receipt.room 
AND unit.date = receipt.date) INNER JOIN room_details ON(room.type = room_details.id) 
INNER JOIN user ON(user.room = receipt.room)  WHERE room.room = '$room' AND  unit.date IN 
 ('$findtoo[date]') GROUP BY receipt.date ORDER BY receipt.date DESC LIMIT 1 ");
$other23 = mysqli_fetch_array($ot);

?>
<?php if(!empty($history['date'])) :?>
<table border="1" width="100%">
    <tr>
        ...
        <td colspan="2">
            ...
            <div>ช่วงวันที่จดมิตเตอร์:
                <?php
                list($yo,$mo,$do) = explode('-', $history['date']); echo $do . '/' . $mo . '/' . date($yo+543);
                ?>
                ถึง
    ...
    <tr>
        <td colspan="3">
            <?php
            $userid = mysqli_query($con, "SELECT * FROM user INNER JOIN room on(user.room = room.room)
             INNER JOIN room_details on(room_details.id = room.type) WHERE room.room = '$room' ");
    ...
    <tr>
        ...
        <td style="text-align: right">
            <?php $sumElectricity = ($findtoo['last_unit_electricity'] - $history['last_unit_electricity']) * $findtoo['unitPerElectricity'];
    ...
    <tr>
        ...
        <td style="text-align: right">
            <?php $sumWater = ($findtoo['last_unit_water'] - $history['last_unit_water']) * $findtoo['unitPerWater'];
...
<table border="1" width="100%">
    <tr>
        ...
        <td colspan="2">
            ...
            <div>ช่วงวันที่จดมิตเตอร์:
                <?php
                list($yo,$mo,$do) = explode('-', $user2['rental_start_date']); echo $do . '/' . $mo . '/' . date($yo+543);
                ?>
                ถึง
                <?php list($yf,$mf,$df) = explode('-', $findtoo['date']); echo $df . '/' . $mf . '/' . date($yf+543);?>
            </div>
