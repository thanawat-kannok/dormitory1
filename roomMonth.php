...
<body style="background-color:#708090;">
        ...
        <div class="container d-flex align-items-center justify-content-between">
            ...
            <nav id="navbar" class="navbar">
                <ul>
                    ...
                    <li class="dropdown"><a href="#" class="dropdown"><?php echo $_SESSION['fname']; ?>( ผู้ใช้ )<i
    ...
    <div class="card" style=" margin-top: 60px;" id="MultiCarousel">
      ...
      if ($_POST['search1'] != "all") :
        ...
        type_name,bed_type.name AS bname,bed_type.size,bed_type.detal,room_picture.room_picture,room_category.name
         AS cname, room_details.floor,room_category.status FROM room_type INNER JOIN room_details ON(room_type.id = room_details.type) 
         INNER JOIN bed_type ON(room_details.bed = bed_type.id) INNER JOIN room_category ON(room_details.reservation = room_category.id)
          ...
          ON(picture_details.picture = room_picture.id) WHERE room_details.reservation = 2 and room_details.floor = $_POST[search1]
           GROUP BY room_details.id ORDER by room_details.floor asc");
      ...
      if ($_POST['search1'] == "all") :
        ...
        AS cname, room_details.floor,room_category.status FROM room_type INNER JOIN room_details 
        ON(room_type.id = room_details.type) INNER JOIN bed_type ON(room_details.bed = bed_type.id) INNER JOIN 
        room_category ON(room_details.reservation = room_category.id) INNER JOIN picture_details 
        ON(room_details.id = picture_details.id_type) INNER JOIN room_picture ON(picture_details.picture = room_picture.id) 
        WHERE room_details.reservation = 2 GROUP BY room_details.id ORDER by room_details.floor asc");
        while ($row = $qry->fetch_assoc()) :
    ...
    <?php
      if (isset($_POST['search1']) == NULL) :
        ...
        INNER JOIN room_category ON(room_details.reservation = room_category.id) INNER JOIN picture_details 
        ON(room_details.id = picture_details.id_type) INNER JOIN room_picture ON(picture_details.picture = room_picture.id)
         WHERE room_details.reservation = 2 GROUP BY room_details.id ORDER by room_details.floor asc");
        while ($row = $qry->fetch_assoc()) :
        ...
        <div class="col-md-5" data-toggle="modal" data-target=".bd-example-modal-lg">
            ...
            ?>
                    <div class="item">
                        <div class="pad15">
                            <img src="assets/img/<?php echo $rowrd['room_picture'] ?>" 
                                onclick="openModal();currentSlide(1)" class="hover-shadow cursor respon">
                        </div>
                    </div>
                    <?php endwhile ?>
                </div>
